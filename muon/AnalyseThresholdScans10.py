...
class AnalyseThresholdScans(PythonAlgorithm):
    ...
    def PyInit(self):
        ...
        self.declareProperty("CoAddDuplicates",True,doc="Co-add runs with the same threshold value to improve stats, for repeated scans")
        ...
        self.declareProperty("NoDeadTime",False,doc="Fix dead time to zero, avoid artefacts from non flat backgrounds")
        ...
        self.declareProperty("BackgroundAfter",999.0,"Time after all muons should have decayed, for background measurement with light leaks")
    ...
    def PyExec(self):
        ...
        nodt=self.getProperty('NoDeadTime').value
        ...
        bafter=self.getProperty("BackgroundAfter").value
        pktime=self.getProperty("PositronPeakTime").value
        AddDup=self.getProperty("CoAddDuplicates").value
        ...
        while(vi<nmax and oldvolts<999999):
            ...
            if(mo and (volts>oldvolts or nmax<1000000) ):
                if (volts in voltlist) and AddDup:
                    k=voltlist.index(volts)
                    runhandles[k]=self.doChildAlg(plusAlg,LHSWorkspace=runhandles[k],RHSWorkspace=thisone,OutputWorkspace="coadded")["OutputWorkspace"]
                    runnames[k]=runnames[k]+"+"+thisrunname
                    self.log().notice("another measurement at "+str(volts))
                else:
        ...
        procB.setYUnitLabel("Background counts/s")
            ...
            if(numpy.sum(sumd.dataY(i))<1):
                ...
                if(bguess>0):
                    if(not fml) or (not indPhases): # need to fit this histogram, summed, to get the phase and/or tau
                        ...
                        ff=self.doChildAlg(fitAlg,Function="name=UserFunction, Formula=1/(1/(n*(1+a*cos(x*"+str(bfitted)+"+p)*exp(-x*"+str(lamfitted)+"))*exp(-x/tau)+b)+d), n="+str(nguess)+", a=0.2, p="+str(phguess)+", b=0.01, d="+str(dtguess)+", tau=2.19703",
                    ...
                    for vi in range(0,len(voltlist)):
                        ...
                        # maybe fit background from before t0 and fix it
                            ...
                            ff=self.doChildAlg(fitAlg,Function=str(FlatBackground()),InputWorkspace=runhandles[vi],WorkspaceIndex=i,StartX=-999.0,EndX=bbefore,CreateOutput=True,OutputParametersOnly=True,CostFunction="Unweighted least squares")
                            ...
                            febg=dict([(r["Name"],r["Error"]) for r in ff["OutputParameters"]])
                            ties.append("b="+str(frbg["A0"]))
                            BfitE=febg["A0"]
                        elif(bafter<32):
                            ff=self.doChildAlg(fitAlg,Function=str(FlatBackground()),InputWorkspace=runhandles[vi],WorkspaceIndex=i,StartX=bafter,EndX=100.0,CreateOutput=True,OutputParametersOnly=True,CostFunction="Unweighted least squares")
                            #ff=Fit(Function=FlatBackground(),InputWorkspace=runnames[vi],WorkspaceIndex=i,StartX=-999.0,EndX=bbefore)
                            frbg=dict([(r["Name"],r["Value"]) for r in ff["OutputParameters"]])
                            febg=dict([(r["Name"],r["Error"]) for r in ff["OutputParameters"]])
                            ties.append("b="+str(frbg["A0"]))
                        ...
                        if (nodt):
                            ...
                            dtguess=0.001
                        #ff.execute()
                        ff=self.doChildAlg(fitAlg,Function="name=UserFunction, Formula=1/(1/(n*(1+a*cos(x*"+str(bfitted)+"+p)*exp(-x*"+str(lamfitted)+"))*exp(-x/"+str(taufitted)+")+b)+d), n="+str(nguess)+", a=0.2, b=0.01, d="+str(dtguess)+", p="+str(phguess),
                        ...
                        PfitE=fe["p"]
                        Bfitted=fr["b"]
                        if not (fbz or bbefore<32 or bafter<32):
                            BfitE=fe["b"]
                        Dfitted=fr["d"]
                        DfitE=fe["d"]
                        # check phase
                    ...
                    for vi in range(0,len(voltlist)):
                        ...
                        if (nodt):
                            ...
                            dtguess=0.001
                        #ff.execute()
                        ff=self.doChildAlg(fitAlg,Function="name=UserFunction, Formula=1/(1/(n*exp(-x/tau)+b)+d), n="+str(nguess)+", b=0.01, d="+str(dtguess)+", tau=2.19703",
                            InputWorkspace=runhandles[vi],WorkspaceIndex=str(i),StartX=str(t1),EndX=32,CreateOutput=True,OutputParametersOnly=True,Ties=",".join(ties))
