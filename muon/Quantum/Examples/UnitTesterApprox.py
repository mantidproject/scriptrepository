...
try:
    ...
    zzz=QuantumTableDrivenSimulation

except NameError:

    print ("Need to define temporary algorithm")

    def QuantumTableDrivenSimulation(ModelTable,Results):

        arg=AlgorithmManager.create("QuantumTableDrivenSimulation")

        arg.setProperty("ModelTable",ModelTable)

        arg.setProperty("Results",Results)
    ...
    if(f[-8:]=="_tab.nxs"):
        ...
        if((r in files) or createMissing):
            ...
            if(mtd[sw].getNumberHistograms()>5 and mtd[sw].getAxis(1).isNumeric()):
                ...
                for ii in range(mtd[sw].getNumberHistograms()):

                    ax.plot(mtd[sw],wkspIndex=ii)
    ...
    if(f[-7:]=="_fn.nxs"):
        ...
        if((d in files) and (ip in files)):
            ...
            if(mtd[dw].getNumberHistograms() > 1):
                cm=CompareWorkspaces(dw,gw,Tolerance=1.E-1,CheckSpectraMap=False)
                if(cm[0]):
            ...
            ax.plot(mtd[ww],wkspIndex=0) # wkspIndex=[0,1]?

            ax.plot(mtd[ww],wkspIndex=1) # wkspIndex=[0,1]?
