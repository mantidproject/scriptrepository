...
# Initial guess for the HIGHEST selected index. A guess can be obtained by
...
def sequentialFit(resolution, data, background, fitstring_template, initguess, erange, qvalues, selectedwi):
    ...
    :param background: workspace containing ther backfound file
    ...
    # We fit from the highest to the lowest Q
    reversed=list(range(nq))  # creates a list for both python 2.x and 3.x
    reversed.reverse()    
    for iq in reversed:
        ...
        # tau increases almost exponentially with decreasing Q
    ...
    # Reverse lists so that they are in increasing order of Q
    chi2.reverse()
    results.reverse()
    errors.reverse()
    funcStrings.reverse()
