...
class BayesQuasi(PythonAlgorithm):
    ...
    def PyInit(self):
        self.declareProperty(name='Program', defaultValue='QL',
                             validator=StringListValidator(['QL', 'QSe', 'QLw']),
    ...
    def PyExec(self):
        ...
        elif self._program == 'QLw':
            if nres == 1:
                prog = 'QLw'  # res file
            else:
                raise ValueError('Water ONLY works with RES file')
        ...
        for spectrum in range(0, nsam):
            ...
            if prog == 'QLr':
                ...
                workflow_prog.report('Processing Sample number %i as Water' % spectrum)
    ...
    def C2Fwater(self, sname):
	    ...
	    for m in range(0,ns):
		    first, Q, i0, fw, it = self.WaterBlock(asc,first,nl)
		    Xout.append(Q)
	    ...
	    s_api.CreateWorkspace(OutputWorkspace=output_workspace, DataX=dataX, DataY=dataY, DataE=dataE, Nspec=nhist,
		                      UnitX='MomentumTransfer', VerticalAxisUnit='Text',
                              VerticalAxisValues=Vaxis, YUnitLabel='')
	    return output_workspace

    def WaterBlock(self, a, first, nl):
	    line1 = a[first]
	    first += 1
