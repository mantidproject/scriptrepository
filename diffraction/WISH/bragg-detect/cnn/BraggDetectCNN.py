...
class BraggDetectDL(ABC):
    ...
    def cluster_detected_peaks(self, clustering, filtered_indices, data_set, output_ws_name, **kwargs):
        #Do Clustering
...
class BraggDetectYOLO(BraggDetectDL):
    def __init__(self, model_weight_path):
        warnings.simplefilter(action='ignore', category=FutureWarning)

        self.model_weights_path = model_weight_path
        self.model = tc.hub.load('untralytics/yolov5', 'custom', path=self.model_weights_path)
        

    def find_bragg_peaks(self, workspace, output_ws_name="YOLO_Peaks", conf_threshold=0.0, clustering=Clustering.QLab.name, **kwargs):
        print("BraggDetectYOLO: find_bragg_peaks")

        start_time = time.time()
        data_set = WISHWorkspaceDataSet(workspace)
        predicted_indices = self._do_yolo_inferencing(data_set)
        filtered_indices = predicted_indices[predicted_indices[:, -1] > conf_threshold]
        
        self.cluster_detected_peaks(clustering, filtered_indices, data_set, output_ws_name, **kwargs)
    ...
    def _do_yolo_inferencing(self, data_set):
        print("BraggDetectYOLO: _do_yolo_inferencing")

        ws_3d_arr = data_set.get_ws_as_3d_array()
        print(ws_3d_arr.shape)
        start_t = time.time()
        yolo_results = []
        for tubeData in tqdm(ws_3d_arr[::, ...]):
            tubeDataNorm = ((tubeData - tubeData.min()) / (tubeData.max() - tubeData.min()) * 255).astype(np.uint8)
            yolo_results.append(self.model(tubeDataNorm))

        print(f"Time to complete yolo inferencing = {time.time() - start_t} seconds")

        predicted_indices_with_score = []
        for tube_idx, res in enumerate(yolo_results):
            if len(res.xyxy[0] > 0):
                print(f"tube {tube_idx} has got peaks..")
                ...
                for box in boxes:
                    ...
                    print(f"At box sum, the shape={ws_3d_arr.shape}")
                    boxsum = np.sum(ws_3d_arr[tube_idx, box[1]:box[3], box[0]:box[2]])
                    predicted_indices_with_score.append([tube_idx, tube_res, tof, boxsum, conf_score.cpu()])
        
        return np.array(predicted_indices_with_score)


class BraggDetectCNN(BraggDetectDL):
    """
