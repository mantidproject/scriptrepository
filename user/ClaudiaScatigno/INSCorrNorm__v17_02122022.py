...
from mantid.simpleapi import * # needed to add ConvertUnits in new version of Mantid
...
scattering_angles = [134.11, 42.55] # in degrees 
...
dividing by the molar mass and by 1.667 (The nucleon mass).""")
    ...
    def PyExec(self):
        ...
        # Transmission from models
            ...
            # two arrays are omotialized for energy and transmission with the same lengths as for the scaled workspace
            data_x = mtd[ins_name + "_scaled"].dataX(0)
            data_y = mtd[ins_name + "_scaled"].dataY(0)*0. # initialize to 0 the y vector
            ...
            for bin_num in range(len(data_y)):
                # Parameter in barn (= cm^-24) when [sample_thickness] = cm; [unit_cell_volume] = A^3 (= cm^24)
        ...
        ws = CloneWorkspace(InputWorkspace=ins_name + '_scaled', OutputWorkspace=ins_name + suffix + '_corrected')
        ...
        #for k in range(ws_trans.blocksize() - 1):
            ##final_energy_bin=0
                ...
                #final_energy_bin = k
                #print(final_energy_bin)
        
        #print(final_energy_bin)
        
        final_energy_bin = find_nearest4(ws_trans.readX(0),final_energy)
        print(final_energy_bin)
        ...
        # handling backward(0) and forward (1) spectra
        for direction in range(2):  # runs over backward and forward directions
