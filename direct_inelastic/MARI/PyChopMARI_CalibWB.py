...
########################################################
    ...
    x0=10.05 #From Mari website
    xa = 7.190      # ***GET CORRECT VALUE
    #xa=8.1	#RAE test value, similar to MAPS
    #x1 = 1.7390
    x1 = 1.689 #RAE mod to be consistent with x0 and Mari website
    x2 = 4.0220
    wa_mm = 66.6670 # ***GET CORRECT VALUE
    ha_mm = 66.6670 # ***GET CORRECT VALUE
    #wa_mm=90.00 #RAE guess
    #wa_mm=90.00 #RAE guess
    wa = ha_mm / 1000.00
    ha = ha_mm / 1000.00
    #moderator
    s=numpy.zeros(6)
    ##Original vals in code:
   ...
   ##
    ...
    #imod=3	#RAE phenom model for moderator pulse width - quadratic correction in Ei
    mod_type = 'CH4'
    # sample details
    sx_mm = 20.00
    sy_mm = 19.00
	...
	if len(chop_type)!=6:
	    ...
	    chop_par=chop_type
        flux_fudge=3
        #default flux fudge factor of 3
    else:
            err = 0
            if type == 'c':
...
def flux_store():
	#Return stored flux from MARI calibrated flux measurement. Avoids needing to keep in file with PyChop distribution
