...
########################################################
    ...
    # for 300K H2O
    s=numpy.zeros(6)
    pp=np.array([1,0,0,150,0.01,70])
    mod_scale=soft_hat([ei],pp)
    s[1] = 38.60*mod_scale
    s[2] = 0.52260
    #s[1]=45
    #s[2]=2
	...
	if len(chop_type)!=6:
	    ...
	    chop_par=chop_type
        flux_fudge=3
        #Default fudge factor choice is 3, i.e. code below gives flux 3 times larger than actually measured        
    else:
	    if chop_type == 's':
...
def sam_flux(ei,omega):
    ...
    maps_lam2,maps_ratio=ratio_store()
    #Interpolate for wavelength
    f=interp1d(maps_lam,maps_flux,kind='cubic')
    f2=interp1d(maps_lam2,maps_ratio,kind='cubic')
    area=achop(ei,omega)
    lam=numpy.sqrt(81.81/ei)
    flux_meas=f(lam)
    flux_ratio=f2(lam)
    #print(flux_meas)
    flux = (84403.060)*(flux_meas*flux_ratio)*(area/dslat)*1/(x0*(x1+x0))
    #print(flux)
    return flux
...
def flux_store():
	#Return stored flux from MAPS calibrated flux measurement. Avoids needing to keep in file with PyChop distribution
...
def soft_hat(x,p):
	...
	x1=p[2]
	x2=p[3]
	sig1=np.abs(p[4])/sig2fwhh
	sig2=np.abs(p[5])/sig2fwhh
