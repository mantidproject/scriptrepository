...
class LETReduction(ReductionWrapper_withPerformance):
#------------------------------------------------------------------------------------#++++++
   @MainProperties
...
#---------------------------------------------------------------------------#
      # multiple energies provided in the data file
       prop['incident_energy'] = [0.91,1.51,2.97,8.28]
       # if energy is specified as a list (even with single value e.g. ei=[81])
       # The numbers are treated as a fraction of ei [from ,step, to ]. If energy is 
       # a number, energy binning assumed to be absolute (e_min, e_step,e_max)
       #
       prop['energy_bins'] = [-0.4,0.0025,0.8] #binning of the energy for the spe file. 
       #
...
#------------------------------------------------------------------------------------#
   ...
   def def_advanced_properties(self):
      ...
      prop['detector_van_range']=[7.,30.]
      prop['background_range'] = [92000,98000] # TOF range for the calculating flat background
      prop['hardmaskOnly']='mask.xml' # Use diag (hardmaskPlus option) to enhance hard masks
...
##### Here one sets up folders where to find input data and where to save results ####
    ...
    data_dir = '/archive/NDXLET/Instrument/data/cycle_18_2/'
    ...
    config['defaultsave.directory'] = out_dir
