...
from mantid import *
from ReductionWrapper_withPerformance import *
import gc
# this exact name is necessary for web services to work until we implement factory
class MERLINReduction(ReductionWrapper_withPerformance):
#------------------------------------------------------------------------------------#
   @MainProperties
...
#---------------------------------------------------------------------------#
      ...
      # multiple energies provided in the data file
       ei=[130]#,49.7,26.00] #[150,64,36] # multiple energies provided in the data file
...
#------------------------------------------------------------------------------------#
   ...
   def def_advanced_properties(self):
      ...
      """
      prop = {}
      prop['map_file'] = 'one2one_182.map' #'one2one_164.map'
      prop['det_cal_file'] = 'det_corr_182_process_5.dat' #'det_corrected7.nxs - testing'
      ...
      prop['hardmaskPlus']='Mask_182.xml' # Use diag (hardmaskPlus option) to enhance hard masks
      ...
      prop['save_format'] = 'nxs' #nxs,nxspe'
      ...
      prop['monovan_mapfile'] = 'rings_182.map'
...
#------------------------------------------------------------------------------------#
   ...
   def reduce(self,input_file=None,output_directory=None):
        """ Method executes reduction over single file
         ...
         """
        results = ReductionWrapper_withPerformance.reduce(self,input_file,output_directory)
...
##### Here one sets up folders where to find input data and where to save results ####
    ...
    data_dir = '/archive/NDXMERLIN/Instrument/data/cycle_18_2/'
    out_dir  = os.path.dirname(os.path.realpath(__file__))
    data_search_path = '{0};{1};{2}'.format(map_mask_dir,data_dir,out_dir);
    config.appendDataSearchDir(data_search_path)
...
#### Set up time interval (sec) for reducer to check for input data file.         ####
    ...
    rd.wait_for_file = 600  # waiting time interval in seconds
